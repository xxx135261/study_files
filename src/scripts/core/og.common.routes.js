    ...
    obj: function () {
        ...
        return routes = $.extend(true, window.RouteMap, {
            ...
            init: function () {
                ...
                $('a[href]').live && $('a[href]').live('click', function (e) {
                ...
                $(function () { // in addition to binding hash change events to window, also fire it onload
                    var common = og.views && og.views.common, is_child, opener_og, parent_api, parent_data, api = og.api.rest;
                    if (common)
                    {
                        $('.OG-js-loading').hide();
                    ...
                    // window.parent to window, we use document because IE8 doesn't know true from false)  检查父窗口的OG脚本是否完整。
                    is_child = (window.parent.document !== window.document) //|| (window.opener && window.opener.og))
                        && (window.parent.og && window.parent.og.views);
