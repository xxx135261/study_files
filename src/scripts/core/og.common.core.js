...
(function ($, window, document, undefined) {
    console.log(window);
    var default_obj = {
            debug: window.location.hash.split('/').some(function (val) {
                return val === 'debug=true';
            })
        },
        check_dependencies,
        /** @private */
    ...
    $.extend({
        ...
        register_module: function (module) {
            ...
            (levels = name.split('.')).reduce(function (acc, val) {
                return acc[val] || (acc[val] = {});
            }, top_level);
            // drill down to the second-to-last level
            last = levels.pop();
            last_parent = levels.reduce(function (acc, val) {
                return acc[val];
            }, top_level);
...
//     // com.hundsun.dev.warn(['UNCAUGHT ERROR!', message, file, agent].join('\n'));
//     console.log(['UNCAUGHT ERROR!', message, file, agent].join('\n'));
