 ...
 */
$.register_module({
    name: 'com.hundsun.pas.api.text',
    dependencies: ['com.hundsun.pas.api.common'],
    obj: function () {
        var html_cache = {}, module = this, api, common = com.hundsun.pas.api.common,
            loading_start = common.loading_start, loading_end = com.hundsun.pas.api.common.loading_end,
            path = function (root, extension, page) {
                // return !page ? null : root + [page.split('.').slice(1, -1).join('/'), page.toLowerCase() + extension]
                //         .join('/').replace(/\s/g, '_');
                return ".." + (root + page + extension);
            },
