        ...
        return { // all requests that begin with /views
            ...
            put: function (config, promise) {
                ...
                var promise = promise || new common.Promise, root = this.root, method = [root], data = {}, meta,
                    fields = [
                        'viewdefinition', 'aggregators', 'aggDefUniqueId','providers', 'portfolio', 'valuation', 'version', 'correction', 'blotter',
                        'benchId', 'calcTimeParam', 'securityType'
                    ],
                    api_fields = [
                        'viewDefinitionId', 'aggregators', 'aggDefUniqueId', 'marketDataProviders', 'portfolioIds', 'valuationTime',
                        'portfolioVersionTime', 'portfolioCorrectionTime', 'blotter', 'benchId', 'calcTimeParam', 'securityType'
                    ];
                ...
                data['portfolioIds'] = config.portfolio;
                    ...
                    viewports: {
                        ...
                        put: function (config) {
                            ...
                            fields.forEach(function (key, idx) {
                                data[api_fields[idx]] = config[key];
                            });
                            data['clientId'] = api.id;
