        ...
        return { // all requests that begin with /valuerequirementnames
            ...
            get: function (config) {
                ...
                meta = check({
                    ...
                    required: [{all_of: ['viewDefinitionId', 'portfolioIds']}]});
                method[1] = config.viewDefinitionId;
                method[2] = config.portfolioIds;
            ...
            put: function (config, promise) {
                ...
                var promise = promise || new common.Promise, root = this.root, method = [root], data = {}, meta,
                    fields = [
                        'viewdefinition', 'aggregators', 'providers', 'portfolio', 'valuation', 'version', 'correction', 'blotter'
                    ],
                    api_fields = [
                        'viewDefinitionId', 'aggregators', 'marketDataProviders', 'portfolioIds', 'valuationTime',
                        'portfolioVersionTime', 'portfolioCorrectionTime', 'blotter'
                    ];
                if (!api.id) return setTimeout((function (context) {                    // if handshake isn't
                    return function () {og.api.rest.realtime.put.call(context, config, promise); }; // complete, return a
                })(this), STALL), promise;                                              // promise and try again
                meta = check({
                    bundle: {method: root + '#put', config: config},
                    required: [{all_of: ['viewdefinition']}]
                });
                meta.type = 'POST';
            ...
            status: {
                root: 'realtimes/{{view_id}}/pauseOrResume',
            ...
            realtime :{
                viewports :{
                    structure:{
                        root: 'realtimes/{view_id}/viewports',
                        get: function (config) {
                            config = config || {};
                            var root = this.root, method = root.split('/'), data = {}, meta;
                            meta = check({
                                bundle: {method: root + '#get', config: config},
                                required: [{all_of: ['view_id','data_type']}]
                            });
                            method[1] = config.view_id;
                            method[2] = config.data_type;
                            return api.request(method, {data: data, meta: meta});
                        },
                        put: function (config) {
                            config = config || {};
                    ...
                    root: 'realtimes/{{view_id}}/{{grid_type}}',
                    get: function (config) {
                        config = config || {};
                        var root = this.root, method = root.split('/'), data = {}, meta;
                        meta = check({
                    ...
                    put: function (config) {
                        ...
                        data.version = promise.id;
                        method[1] = config.view_id;
                        method[2] = config.grid_type||'allData';
                        if (config.viewport_id) {// use the promise id as viewport_version
                            meta.type = 'PUT';
                            method.push(config.viewport_id);
                        }
