...
$.register_module({
    ...
    obj: function () {
        ...
        var module = this, live_data_root = module.live_data_root, api, warn = og.dev.warn,
            ...
            has_id_search = {
                configs: true, exchanges: true, holidays: true, portfolios: true, positions: true,
        ...
        var cache_get = function (key) {
            return common.cache_get(api.name + key);
        };
        var cache_set = function (key, value) {
            return common.cache_set(api.name + key, value);
        };
        var cache_del = function (key) {
            return common.cache_del(api.name + key);
        };
        ...
        var default_get = function (fields, api_fields, config) {
            ...
            var root = this.root, method = [root], data = {}, meta,
                ...
                field_search = fields.some(function (val) {
        ...
        var request = function (method, config, promise) {
            ...
            var send = function () {
                ...
                outstanding_requests[promise.id].ajax = $.ajax({
                    ...
                    // add by Rencc in 20171109.加这个参数才可以跨域时设置和携带cookie
                    ...
                    dataType: config.meta.datatype || 'text',
                    processData: typeof config.meta.processData == 'boolean' ? config.meta.processData : true,
                    contentType: config.meta.contentType != undefined ? config.meta.contentType : 'application/x-www-form-urlencoded',
                    timeout: config.meta.timeout || (is_get ? time_out_soon : time_out_forever),
                    // meta.sync, true: 同步, false: 异步
                    async: !config.meta.sync ? true : false,
                    ...
                    success: function (data, status, xhr) {
                        ...
                        if (cache_for = config.meta.cache_for)
                            cache_set(url, result), setTimeout(function () {
                                cache_del(url);
                            }, cache_for);
                        config.meta.handler(result);
                        promise.deferred.resolve(result);
            ...
            if (is_get) { // deal with client-side caching of GETs
                if (cache_get(url) && typeof cache_get(url) === 'object') return setTimeout((function (result) {
                    ...
                    return function () {
                        config.meta.handler(result), promise.deferred.resolve(result);
                    };
            ...
            if (is_delete) api.registrations = api.registrations
                .filter(function (reg) {
                    return reg && !~reg.method.join('/').indexOf(method.join('/'));
                });
        ...
        api = {
            ...
            deregister: function (promise) {
                api.registrations = api.registrations.filter(function (val) {
                    return val.id !== promise.id;
                });
            },
            disconnected: false,
