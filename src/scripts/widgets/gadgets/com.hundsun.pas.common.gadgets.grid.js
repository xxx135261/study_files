 ...
 */
import {BMP} from './../../core/util/jsbmp';

$.register_module({
    name: 'com.hundsun.pas.common.gadgets.Grid',
    dependencies: [],
    obj: function () {
        var module = this, row_height = 21, title_height = 31, set_height = 24, logging = 'logLevel',
            templates = null, default_col_width = 175, HTML = 'innerHTML', compile = Handlebars.compile,
            scrollbar = com.hundsun.pas.common.gadgets.scrollbar_size,
            do_not_expand = {
                DOUBLE: null,
                FUNGIBLE_TRADE: null,
                NODE: null,
        ...
        var css = '{{id}} {\n' +
            ...
            '    user-select: none;\n' +
            '}\n' +
            '{{id}} .OG-g-b-scroll {width: {{viewport_width}}px; left: {{scroll_left}}px; height: {{height}}px;}\n' +
            '{{id}} .OG-g-h-scroll {width: {{viewport_width}}px; height: {{header_height}}px; left: {{scroll_left}}px}\n' +
            '{{id}} .OG-g-b-scroll .OG-g-rows {width: {{scroll_width}}px; height: {{scroll_height}}px}\n' +
            '{{id}} .OG-g-b-fixed .OG-g-rows {height: {{fixed_height}}px}\n' +
            '{{#if scroll_bg}}\n' +
            '    {{id}} .OG-g-cell {border: 0px;}\n' +
            '    {{id}} .OG-g-b-scroll .OG-g-rows {background: url({{scroll_bg}}); background-repeat: repeat-y;}\n' +
            '    {{id}} .OG-g-b-fixed .OG-g-rows {background: url({{fixed_bg}}); background-repeat: repeat-y;}\n' +
            '{{/if}}\n' +
            '{{id}} .OG-g-b-scroll .OG-g-row {width: {{scroll_width}}px}\n' +
            '{{id}} .OG-g-b-fixed {width: {{fixed_width}}px; height: {{height}}px; overflow-y: scroll;}\n' +
            '{{id}} .OG-g-h-fixed {width: {{fixed_width}}px}\n' +
            '{{id}} .OG-g-b {top: {{header_height}}px;}\n' +
            '{{id}} .OG-g-h-cols {top: {{set_height}}px}\n' +
            '{{id}} .OG-g-row, .OG-g-cell {height: {{row_height}}px; line-height: {{row_height}}px;}\n' +
            '{{id}} .OG-g-cell.og-cell-cellhighlight {height: {{row_height_min_one}}px;}\n' +
            '{{id}} .OG-g-rest {\n' +
            '    position: absolute;\n' +
            '    background: #fff;\n' +
            '    top: {{rest_top}}px;\n' +
            ...
            '}\n' +
            '{{id}} .OG-g-b-fixed .OG-g-rest {width: {{fixed_width}}px;}\n' +
            '{{id}} .OG-g-b-scroll .OG-g-rest {width: {{scroll_width}}px;}\n' +
            '{{#each sets}}\n' +
            '    {{prefix}} .OG-g .s{{index}} {left: {{left}}px; right: {{right}}px;}\n' +
            '{{/each}}\n' +
            '{{#each columns}}\n' +
            '    {{prefix}} .OG-g .c{{index}} {left: {{left}}px; right: {{right}}px;}\n' +
            '    {{prefix}} .OG-g .c{{index}}.OG-g-cell.og-cell-cellhighlight {left: {{left_min_one}}px;}\n' +
            '{{/each}}\n' +
            '{{id}} .OG-g .OG-g-b-scroll .OG-g-rows .OG-g-row .OG-g-cell.og-cell-cellhighlight,\n' +
            '{{id}} .OG-g .OG-g-b-fixed .OG-g-rows .OG-g-row .OG-g-cell.og-cell-cellhighlight {border: 1px solid #000;}\n' +
            '{{id}} .OG-g .OG-g-b-scroll .OG-g-rows .OG-g-row .OG-g-cell.og-cell-cellhighlight:first-child {left: 0;}\n' +
            '{{id}} .OG-g .OG-g-b-scroll .OG-g-rows .OG-g-row .OG-g-cell.og-cell-cellhighlightinplace,\n' +
            '{{id}} .OG-g .OG-g-b-fixed .OG-g-rows .OG-g-row .OG-g-cell.og-cell-cellhighlightinplace {background-color: #eee;}\n' +
            '{{id}} .OG-g .node {width: 8px; height: 8px; display: inline-block;}\n' +
            '{{id}} .OG-g .node.collapse:before {\n' +
            // '    background: url("./../../../images/plugin/main-sprite-sheet.png") repeat scroll 0 -25px transparent;\n' +
            '    content: "";\n' +
            '    cursor: pointer;\n' +
            '    height: 9px;\n' +
            '    position: absolute;\n' +
            ...
            '    width: 9px;\n' +
            '}\n' +
            '{{id}} .OG-g .node.expand:before {\n' +
            // '    background: url("./../../../images/plugin/main-sprite-sheet.png") repeat scroll -30px -25px transparent;\n' +
            '    content: "";\n' +
            '    cursor: pointer;\n' +
            '    height: 9px;\n' +
            '    position: absolute;\n' +
            ...
            '    width: 9px;\n' +
            '}\n' +
            '{{id}} .OG-g .node.loading:before {\n' +
            // '    background: url("./../../../images/plugin/main-sprite-sheet.png") repeat scroll -40px -25px transparent;\n' +
            '    content: "";\n' +
            '    cursor: pointer;\n' +
            '    height: 9px;\n' +
            '    position: absolute;\n' +
            ...
            '}\n' +
            '/* type formatting */\n' +
            '{{id}} .OG-g-b-scroll .OG-g-DOUBLE {\n' +
            '    text-align: right;\n' +
            '    padding-right: {{#if sparklines}}65{{else}}18{{/if}}px;\n' +
            '}\n' +
            '.OG-g-b-fixed .OG-g-DOUBLE {\n' +
            '    text-align: right;\n' +
            '    padding-right: 4px;\n' +
            '}';
        var header = '<div class="OG-g-h-sets" style="box-sizing: content-box;position: relative; width: {{width}}px; padding-right: {{padding_right}}px;">\n' +
            '  {{#each sets}}\n' +
            '    <div class="OG-g-h-set">\n' +
            '      {{#if show_sets}}\n' +
            '        {{#if views}}\n' +
            '          <!-- OPTIONS AND DROPDOWNS -->\n' +
            '          <div class="OG-g-h-set-name s{{index}}">\n' +
            '            <div class="OG-grid-options">\n' +
            '              <!-- SPARKLINES -->\n' +
            '              {{#if show_sparklines}}\n' +
            '                <div class="og-sparklines og-active">\n' +
            '                  <div class="OG-icon og-icon-sparkline"></div><span>ON</span>\n' +
            '                </div>\n' +
            '              {{else}}\n' +
            '                <div class="og-sparklines">\n' +
            '                  <div class="OG-icon og-icon-sparkline"></div><span>OFF</span>\n' +
            '                </div>\n' +
            '              {{/if}}\n' +
            '              <!-- -->\n' +
            '              <div class="og-divider"></div>\n' +
            '              <!-- CSV -->\n' +
            '              <div class="og-csv og-active">\n' +
            '                <div class="OG-icon og-icon-csv"></div><span>CSV</span>\n' +
            '              </div>\n' +
            '              <!-- -->\n' +
            '              <div class="og-divider"></div>\n' +
            '              <!-- DROPDOWN MENU -->\n' +
            '              <div class="og-dropdown {{#if views.list.length}}og-expandable{{/if}}">\n' +
            '                <span>{{views.selected}}</span>\n' +
            '                {{#if views.list.length}}\n' +
            '                  <span class="OG-icon og-icon-down"></span>\n' +
            '                  <div class="og-menu">\n' +
            '                    {{#each views.list}}<div class="og-js-viewchange">{{this}}</div>{{/each}}\n' +
            '                  </div>\n' +
            '                {{/if}}\n' +
            '              </div>\n' +
            '              {{#if show_save}}\n' +
            '              <!--<div class="og-buttons">\n' +
            '                <div class="OG-button-save">save...</div>\n' +
            '              </div>-->\n' +
            '              {{/if}}\n' +
            '            </div>\n' +
            '          </div>\n' +
            '        {{else}}\n' +
            '          <!-- SET NAMES -->\n' +
            '          <div class="OG-g-h-set-name s{{index}}"><div>{{name}}</div></div>\n' +
            '        {{/if}}\n' +
            '      {{/if}}\n' +
            '      <div class="OG-g-h-cols">\n' +
            '        {{#each columns}}\n' +
            '            <div class="OG-g-h-col c{{index}} {{#if ../reorder}}reorder{{/if}}" data-index="{{index}}"\n' +
            '                title="{{../name}}: {{name}} {{description}}">{{name}}\n' +
            '              <div class="OG-g-h-resize resize"></div>\n' +
            '            </div>\n' +
            '        {{/each}}\n' +
            '      </div>\n' +
            '    </div>\n' +
        ...
        var container = '<div id="{{id}}">\n' +
            ...
            '</div>';
        var loading = '<div>\n' +
            '  <div class="OG-g-loading-overlay"></div>\n' +
            '  <div class="OG-g-loading">\n' +
            '    {{text}}\n' +
            '    <div class="OG-loader {{error}}"></div>\n' +
            '  </div>\n' +
            '</div>\n';
        var row = '<div>\n' +
            '  <div class="OG-g-rows" style="height: {{holder_height}}px;">\n' +
            '    {{#each rows}}\n' +
            '      <div class="OG-g-row OG-g-{{data_row}}" style="top: {{top}}px;">\n' +
            '        {{#each cells}}\n' +
            '          <div class="OG-g-cell OG-g-{{type}} c{{column}}{{#error}} og-error{{/error}}\n' +
            '          {{#highlight}} og-cell-{{../highlight}}{{/highlight}}\n' +
            '          {{#logging}} og-log og-log-{{../logging}}{{/logging}}">\n' +
            '            {{{value}}}\n' +
            '          </div>\n' +
            '        {{/each}}\n' +
            '      </div>\n' +
            ...
            '  </div>\n' +
            '  {{#if loading}}\n' +
            '    <div class="OG-g-rest" style="top: 0px; opacity: 0; filter: alpha(opacity=0);"></div>\n' +
            '  {{else}}\n' +
            '    <div class="OG-g-rest"></div>\n' +
            '  {{/if}}\n' +
            '</div>';
        ...
        var col_css = function (id, sets, offset) {
            var partial = 0, columns = sets.reduce(function (acc, set) {
                    return acc.concat(set.columns);
                }, []),
                total_width = columns.reduce(function (acc, val) {
        ...
        var col_reorder = (function () {
            ...
            var set_after = function (grid, event) {
                ...
                after = Math.max(
                    ...
                    grid.meta.columns.scan.all.reduce(function (acc, val, idx) {
                        return val < x ? idx : acc;
                    }, 0)
                );
            };
        ...
        var col_resize = (function () {
            ...
            var mouseup = function (grid, event) {
                ...
                if (selection && ~selection.cols.indexOf(index))
                    selection.cols.forEach(function (col) {
                        grid.state.col_override[col] = new_width;
                    });
                else
        ...
        var Grid = function (config) {
            ...
            // com.hundsun.pas.api.text.on('fatal', function () {
        ...
        var init_data = function () {
            ...
            grid.dataman = new (config.dataman || og.analytics.Data)(grid.source, {
                bypass: false,
                label: grid.label
            })
            ...
            //     grid.fire('fatal');
            // })
            // .on('title', function (row_name, col_name, name) {
            //     grid.fire('title', row_name, col_name, name);
            // })
            // .on('types', function (types) {
            //     grid.views = {selected: config.source.type || 'portfolio'};
            //     grid.views.list = Object.keys(types).filter(function (key) {
            //         return !!types[key];
            //     });
        ...
        var init_elements = function () {
            ...
            elements.parent.unbind().html(templates.container({id: grid.id.substring(1)}))
                ...
                .on('click', '.OG-g-h-set-name .og-csv', function (event) {
                    com.hundsun.pas.api.text.views.csv({
                        view_id: grid.dataman.connection.view_id,
                        grid_type: grid.source.type
                    });
                    return false;
                })
                .on('mousedown', function (event) {
                    ...
                    grid.resize();//.selector.clear();//解决树形收缩功能
                ...
                .on('contextmenu', '.OG-g-h-cols', function (event) { // for column headers
                    ...
                    grid.fire('contextmenu', event, null, ['description', 'header', 'type']
                        .reduce(function (acc, key) {
                            return (acc[key] = columns[key + 's'][col]), acc;
                        }, {col: col}));
                    return false;
                })
            ...
            // elements.parent[0].onselectstart = function () {
            //     return false;
            // }; // stop selections in IE
            ...
            (function () {
                var started, pause = 200, jump = function () {
                    grid.fire('scrollend'), viewport.call(grid, function () {
                        grid.busy(false);
                    }), started = null;
                };
            ...
            grid.selector = new Object();// new og.common.grid.Selector(grid)
            //     .on('select', function (selection) {
            //         grid.fire('select', selection);
            //     })
            //     .on('deselect', function () {
            //         grid.fire('deselect');
            //     });
            // if (config.cellmenu) try {
            //     grid.cellmenu = new og.common.grid.CellMenu(grid);
            // }
            // catch (error) {
            //     og.dev.warn(module.name + ': cellmenu failed', error);
            // }
        ...
        var init_grid = function (metadata) {
            var grid = this, config = grid.config, meta = grid.meta = $.extend(metadata);
            // ['show_sets', 'show_views'].forEach(function (key) {
            //     meta[key] = key in config ? config[key] : true;
            // });
        ...
        var render_header = (function () {
                ...
                return {
                    ...
                    sets: sets.map(function (set, idx) {
                        ...
                        return {
                            ...
                            width: columns.reduce(function (acc, col) {
            ...
            return function () {
                ...
                grid.elements.scroll_head.html(templates.header(head_data(grid, columns.scroll, meta.fixed_length, fixed_sets)));
            };
        })();
        var render_rows = (function () {
            var row_data = function (grid, fixed, loading) {
                ...
                for (row = 0; row < row_len; row += 1) {
                    result.rows.push({
                        top: row_height * grid_row++,
                        cells: (cells = []),
                        data_row: data_row = rows[row]
                    });
                    if (fixed) {
                        col = 0;
                        col_end = col_len;
                    ...
                    for (; col < col_end; col += 1) {
                        ...
                        if (data[index] !== undefined) { //catch any histogram and timeseries resize lags
                            cells.push({
                                ...
                                highlight: highlight && highlight.row === data_row && highlight.col === data_col && highlight.event_type,
            ...
            return function (data, loading, quiet) { // TODO handle scenario where grid was busy when data stopped
                ...
                grid.elements.scroll_body.html(templates.row(row_data(grid, false, loading)));
                grid.updated(+new Date);
                if (loading) {
                    if (!grid.elements.notified)
                        grid.elements.main.append(grid.elements.notified = $(templates.loading({text: 'waiting for data...'})));
                } else {
        ...
        var set_css = function (id, sets, offset) {
            var partial = 0, total_width = sets.reduce(function (acc, set) {
                return acc.concat(set.columns);
            }, [])
            ...
            return sets.map(function (set, idx) {
                var set_width = set.columns.reduce(function (acc, val) {
                    return val.width + acc;
                }, 0), css;
                css = {
        ...
        Grid.prototype.range = function (selection, expanded) {
            var grid = this, viewport = grid.meta.viewport, row_indices,
                ...
                available = !selection.rows.some(function (row) {
                    return !~viewport.rows.indexOf(row);
                }) && !selection.cols.some(function (col) {
                    return !~viewport.cols.indexOf(col);
                });
            if (!available || !grid.data) return {data: null, raw: null};
            row_indices = selection.rows.map(function (row) {
                return viewport.rows.indexOf(row);
            });
            col_indices = selection.cols.map(function (col) {
                return viewport.cols.indexOf(col);
            });
            ...
            return !expanded ? {data: result, raw: result} : result.every(function (row) {
                return row.pluck('type').every(function (type) {
                    return type in do_not_expand;
                });
