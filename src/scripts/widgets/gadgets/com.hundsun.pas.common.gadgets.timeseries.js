 ...
 */
$.register_module({
    name: 'com.hundsun.pas.common.gadgets.Timeseries',
    dependencies: ['com.hundsun.pas.common.gadgets.manager'],
    obj: function () {
        return function (config) {
            var api = og.api, common = com.hundsun.pas.common, gadget = this, timeseries, selector = config.selector,
                alive = og.common.id('gadget_timeseries'), $selector = $(selector),
            ...
            var CellDataMan = function (row, col, type, source) {
                var dataman = window, format = type === 'CURVE' ? 'CELL' : 'EXPANDED';
                return dataman.cell = new og.analytics.Cells({source: source, single: {row: row, col: col}, format: format}, 'timeseries')
            ...
            if (!config.child) {
                common.manager.register(gadget);
            }
        };
    }
});