 ...
 */
$.register_module({
    name: 'com.hundsun.pas.common.gadgets.Log',
    ...
    obj: function () {
        ...
        return function (config) {
            var gadget = this, alive = og.common.id('gadget_log'), $selector = $(config.selector),
                instantiated, tash, $msg, sbar_size = com.hundsun.pas.common.gadgets.scrollbar_size + 'px',
                cell_options = {
                ...
                gadget.dataman = new og.analytics.Cells(cell_options, 'log').on('data', function (cell) {
                    if (gadget.data = cell.logOutput) {
                        if (!instantiated) $.when(og.api.text({module: 'og.analytics.logger'})).then(function (tmpl) {
                            ...
                            $msg.slideDown().on('click', '.og-link', function () {
                                $msg.slideUp(null, gadget.update);
                            });
                        }
            ...
            if (loading_template) gadget.load(); else loading_template = Handlebars.compile('<div>\n' +
                ...
                '</div>\n');
            gadget.load();
            if (!config.child) com.hundsun.pas.common.gadgets.manager.register(gadget);
        };
    }
});